...
export default function ExpenseCalculatorPage() {
  ...
  const [currentPage, setCurrentPage] = React.useState("form"); // "form" | "calculator"
  ...
  const answer = React.useMemo(() => captchaAnswer(captcha), [captcha]);
  ...
  /* Load items: try admin /api/items first, then fallback to sample files */
  React.useEffect(() => {
    (async () => {
      try {
        // 1) Load admin items from Blob via API
        ...
        // 2) Fallback to sample files in /public
